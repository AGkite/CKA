...
```powershell
multipass launch 24.04 --name k8s-master --cpus 2 --memory 4G --disk 20G
multipass launch 24.04 --name k8s-worker1 --cpus 2 --memory 3G --disk 20G
multipass launch 24.04 --name k8s-worker2 --cpus 2 --memory 3G --disk 20G
multipass list
```
...
| 输出字段 | 说明 |
|----------|------|
| IPv4 | 节点 IP，后面 kubeadm init 和 hosts 都要用 |
| State | Running 表示 VM 已启动 |

记录三台机器 IP，下文用 `<MASTER_IP>` 等占位符表示。
...
| `--apiserver-advertise-address=<MASTER_IP>` | API Server 对外公告的 IP；worker 通过此地址连接 master |
...
| 参数 | 含义 |
|------|------|
| `<MASTER_IP>:6443` | API Server 地址，6443 是默认端口 |
...
| `sed` 替换 IP | VM 内 kubeconfig 的 server 可能是 `127.0.0.1`，WSL 访问需改成 master 的真实 IP |
